...
class RelicBotApp(tk.Tk):
    def __init__(self):
        ...
        self.title(f"Elden Ring Nightreign – Relic Bot v{APP_VERSION}  |  Made by Pulgo")
    ...
    def _on_relic_type_change(self):
        """Switch between Normal and Deep of Night mode.

        Each mode is a completely independent sub-profile. On switch:
        1. Save current UI to _mode_data[old_mode] via JSON deep copy
        2. Load _mode_data[new_mode] into UI via JSON deep copy
        No shared references, no callback chains during swap.
        """
        ...
        try:
            if hasattr(self, "_mode_data") and not _loading:
                ...
                # Save old mode
                self._mode_data[old_mode] = _json.loads(_json.dumps({
                    "criteria":          self.relic_builder.get_state(),
                    "blocked_curses":    list(self._blocked_curses_list),
                    "excluded_passives": list(self._excluded_passives_list),
                }))
                # Load new mode fresh from dict
                md = _json.loads(_json.dumps(self._mode_data.get(rtype, {})))
                ...
                _dormant = _UCATS.get("Dormant Powers", [])
                self._excl_dormant_var.set(
                    bool(_dormant) and all(
                        d in self._excluded_passives_list for d in _dormant if d))
        finally:
        ...
        if os.path.exists(path):
            try:
                ...
                self._log(f"Phase 0 (Setup) swapped to {rtype} sequence"
                          f" ({n} events).")
            except Exception as e:
    ...
    def _batch_loop(self):
        ...
        # Only suggest settings changes when there's clear evidence that
        # the user's SETTINGS caused the issue — not just that failures
        # happened. Avoids false positives on powerful systems where a
        ...
        _gpu_stall_total = _diag_ev.get("input_yield_wait_ms", 0.0)
        _buyq_fails = (_diag_ev.get("buy_qty_unrecoverable", 0)
                       + _diag_ev.get("buy_qty_ocr_fail", 0))
        # Settle contention: only flag if async+workers are the likely cause
